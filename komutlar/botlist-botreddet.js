...
let westrabotlistyetkilisi = db.fetch(`westrabotlistyetkilirol_${message.guild.id}`)
   if(!message.member.roles.cache.has(westrabotlistyetkilisi)) return message.channel.send(`<a:guvenlikk:827113437150052376> Bu komutu kullanabilmen için <@&${westrabotlistyetkilisi}> adlı role sahip olman lazım!`)
	let botisim = args[0]
  let sahip = args[1]
  let sebep = args.slice(2).join(' ');
	let log = westralogkanal // Bot Eklendi / Onaylandı / Rededildi Kanalı
	if (!botisim) return message.channel.send(`<a:guvenlikk:827113437150052376> Botun ID'sini yazmalısın.`).then(x => x.delete({timeout: 3000}))
      if (!sahip) return message.channel.send(`<a:guvenlikk:827113437150052376> Bot sahibinin ID'sini yazmalısın.`).then(x => x.delete({timeout: 3000}))
   if (!sebep) return message.channel.send(`<a:guvenlikk:827113437150052376> Botun onaylanmama sebebini yazmalısın.`).then(x => x.delete({timeout: 3000}))
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    .setColor(`RED`)
    .setFooter(`Spallers BotList Sistemi`)
    .setTimestamp()
    .setDescription(`<a:guvenlikk:827113437150052376> <@${sahip}> adlı kişinin <@${botisim}> adlı botu reddedildi. \n**Sebep:** \`${sebep}\` \n**Reddeden yetkili:** ${message.author}`)
